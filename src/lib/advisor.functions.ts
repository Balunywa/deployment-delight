...
\`\`\`
Allowed fields and values: connectivity ("hub_and_spoke"|"virtual_wan"|"none"), firewall ("Basic"|"Standard"|"Premium"|"none"), bastion, vpnGateway, expressRoute, ddosPlan, identity, securitySubscription, defender, updateManager, serviceHealth, vmBackup ("yes"|"no"), privateDns ("platform"|"none"), monitoring ("azure_monitor"|"third_party"), siem ("sentinel"|"other"), logRetentionDays (number), secondaryRegion (Azure region name or ""), primaryRegion, landingZones (array of "corp","online","local","sandbox"), rbac (array of {persona, role, scope}), policyAdds (array of {id, scope}), customGroups (array of {id, name, parent, archetype: "corp"|"online"|"local"|"sandbox"|"inherit"} — added management groups; parent is a management group role/id), removedGroups (array of "management"|"connectivity"|"identity"|"security"|"decommissioned"), groupNames (object of group id → display name), environments (array like ["dev","test","prod"] — one subscription per environment per customer install), extraSubscriptions (array of {id, name, group, environment}), defaultGroup (group id for new subscriptions, "" for tenant root), workloads (array of {group, id} using ids from workloadLandingZones). Arrays replace the whole list, so include existing items you want to keep.

Microsoft guidance you must respect: don't create management groups for dev/test/prod — environments are separate subscriptions in the same archetype group (CAF resource organization). Create a new management group only when policy or access must differ. Keep the hierarchy to 3-4 levels (Azure allows 6). Only include a patch when a change is actually proposed.`;
