  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Customers · Cloud Delivery" },
      {
        ...
        content: "Customer tenants, deployment models and platform versions.",
      },
    ],
  }),
  component: Customers,
...
function Customers() {
  ...
  return (
    <>
      ...
      {!customers.isLoading && !rows.length && (
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        {rows.map((customer) => {
          ...
          const connection = (
            (customer.customer_connections ?? []) as {
              ...
              <dl className="mt-3 space-y-1 text-xs">
                <div className="flex justify-between gap-2">
                  ...
                  <dd className="truncate font-medium">
                    {prod?.offerings?.name ?? envs[0]?.offerings?.name ?? "—"}
                  </dd>
                </div>
                <div className="flex justify-between gap-2">
                  <dt className="text-muted-foreground">Platform version</dt>
                  <dd
                    className={`mono-num font-medium ${outdated ? "text-warning" : "text-success"}`}
                  >
              ...
              <div className="mt-3 flex items-center justify-between border-t border-border pt-2 text-[11px] text-muted-foreground">
                ...
                <span>
                  {connection?.last_validated_at
                    ? `validated ${relative(connection.last_validated_at)}`
                    : "not validated"}
                </span>
              </div>
            </Link>
