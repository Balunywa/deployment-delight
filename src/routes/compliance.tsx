...
import { describe, relative, titleize } from "@/lib/format";
import { complianceQuery, driftQuery, policyPacksQuery } from "@/lib/queries";

export const Route = createFileRoute("/compliance")({
  head: () => ({
    meta: [
      { title: "Compliance · Cloud Delivery" },
      {
        name: "description",
        content:
          "Versioned policy packs, per-control evidence from deployments, and drift findings across every customer environment.",
      },
      { property: "og:title", content: "Compliance · Cloud Delivery" },
      {
        property: "og:description",
        content: "Policy packs, control evidence and drift across the customer estate.",
      },
    ],
  }),
  ...
  return (
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Panel
          title="Policy packs"
          description="Versioned and immutable once applied"
          bodyClassName="p-0"
        >
            ...
            {(packs.data ?? []).map((p) => (
              <li key={p.id} className="px-4 py-3">
                ...
                <p className="mt-1 text-[11px] text-muted-foreground">
                  {
                    ((p.policy_manifest_json as { controls?: unknown[] } | null)?.controls ?? [])
                      .length
                  }{" "}
                  controls
                </p>
        ...
        <div className="space-y-4 lg:col-span-2">
          <Panel
            ...
            description={
              failsOnly ? "Showing failing and warning controls" : "Showing all evaluated controls"
            }
            actions={
              <button
          ...
          >
            <div className="max-h-[420px] overflow-auto">
                  ...
                  {rows.slice(0, 400).map((c) => {
                    ...
                    return (
                      <tr key={c.id}>
                        ...
                        <td>
                          <span className="mono-num mr-2 text-muted-foreground">
                            {c.control_key}
                          </span>
                          {c.control_name}
                        </td>
                        <td
                          ...
                          title={evidenceText(c.evidence_json)}
                        >
              ...
              {!rows.length && !checks.isLoading && (
                <EmptyState title="No control issues — the estate is fully compliant." />
              )}
            </div>
          </Panel>
          ...
          <Panel
            ...
            description="Desired state versus observed state"
            bodyClassName="p-0"
          >
            <ul className="divide-y divide-border">
              {(drift.data ?? []).map((f) => {
                const env = f.environments as {
                  name: string;
                  customers: { id: string; name: string };
                } | null;
                return (
                  <li key={f.id} className="px-4 py-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Pill tone={f.status === "open" ? "warning" : "neutral"}>
                          {titleize(f.status)}
                        </Pill>
                      </div>
                    </div>
              ...
              {!(drift.data ?? []).length && (
                <li className="px-4 py-6 text-sm text-muted-foreground">No drift recorded.</li>
              )}
            </ul>
          </Panel>
        </div>
...
function evidenceText(evidence: unknown): string {
  ...
  return typeof note === "string" ? note : describe(evidence);
}
