...
export const Route = createFileRoute("/audit")({
  head: () => ({
    meta: [
      { title: "Audit · Azure ISV Deployment Factory" },
      {
        name: "description",
        content: "Immutable audit trail: who did what, to which customer and environment, when, with previous and new values and a correlation ID.",
      },
      { property: "og:title", content: "Audit · Azure ISV Deployment Factory" },
...
function Audit() {
  ...
  const rows = (audit.data ?? []).filter((e) =>
    `${e.event_type} ${e.actor_name ?? ""} ${e.resource_id ?? ""}`.toLowerCase().includes(q.toLowerCase()),
  );
          ...
          <tbody>
            {rows.map((e) => (
              <tr key={e.id}>
                ...
                <td className="text-muted-foreground">{(e.customers as { name?: string } | null)?.name ?? "—"}</td>
                <td className="text-muted-foreground">{(e.environments as { name?: string } | null)?.name ?? "—"}</td>
                ...
                <td>
                  <Pill tone={e.result === "failure" ? "danger" : "success"}>{e.result ?? "success"}</Pill>
                </td>
              </tr>
            ))}
          </tbody>
