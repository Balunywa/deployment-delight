 ...
 */
import { DEFAULT_ANSWERS, LIBRARIES, type Answers, changesFor, hierarchy, libraryFor, shortRef, terraformFor } from "../src/lib/alz/engine";

const out = process.argv[2] ?? "alz-out";

const cases: { name: string; answers: Partial<Answers> }[] = [
  { name: "reference", answers: { ddosPlan: "yes" } },
  { name: "no-ddos-prefixed", answers: { intermediateRootId: "gridworks", intermediateRootName: "GridWorks" } },
  { name: "third-party-monitoring", answers: { monitoring: "third_party", privateDns: "none" } },
  { name: "virtual-wan", answers: { connectivity: "virtual_wan", ddosPlan: "yes" } },
  { name: "cloud-only", answers: { connectivity: "none", siem: "other" } },
];

const summary: Record<string, unknown> = {};
for (const lib of LIBRARIES) {
  for (const c of cases) {
    const answers = { ...DEFAULT_ANSWERS, securityContactEmail: "secops@example.com", ...c.answers };
    ...
    if (changesFor(libraryFor(lib.ref), answers).length || answers.intermediateRootId !== "alz")
      await Bun.write(
        ...
        JSON.stringify({ name: "custom", display_name: "Custom", description: "Generated", dependencies: [{ path: "platform/alz", ref: shortRef(lib.ref) }] }),
      );
    // Expected assignment counts per management group, compared with alzlibtool's composed output in CI.
    summary[`${shortRef(lib.ref)}/${c.name}`] = Object.fromEntries(hierarchy(libraryFor(lib.ref), answers).map((n) => [n.id, n.enforced]));
  }
}
